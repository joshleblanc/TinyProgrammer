...
# Usage: curl -sSL https://raw.githubusercontent.com/joshleblanc/TinyProgrammer/main/setup.sh | bash
...
if [[ ! -f .env ]]; then
  ...
  # Check if key is set (MiniMax or OpenRouter)
  if ! grep -qE '^MINIMAX_API_KEY=sk-' .env && ! grep -qE '^OPENROUTER_API_KEY=sk-' .env; then
    NEEDS_API_KEY=1
  fi
fi
...
if [[ -n "$NEEDS_API_KEY" ]]; then
  echo -e "${Y}====================================================${N}"
  echo -e "${Y}  ACTION REQUIRED: Add your MiniMax API key${N}"
  echo -e "${Y}====================================================${N}"
  echo ""
  echo "1. Get your api key at https://platform.minimax.io/user-center/payment/token-plan"
  echo "2. Edit the .env file:"
  echo "     nano $INSTALL_DIR/.env"
  echo "3. Set MINIMAX_API_KEY=sk-..."
